...
String html() {
  ...
  "<h1>GMpro87 v2.5</h1>"
  "<div class=c>"
  "<div class=d><span>WiFi</span><span>" + String(attack ? "ON" : "OFF") + "</span></div>"
  "<div class=d><span>Beacon</span><span>" + String(beacon ? "ON" : "OFF") + "</span></div>"
  "<div class=d><span>BLE</span><span>" + String(blespam ? "ON" : "OFF") + "</span></div>"
  ...
  "</div>"
  "<div class=c><h3>Networks:</h3>" + netlist[0] + "</div>"
...
void scan() { 
  ...
  netlist[0] = "";
  for (int i = 0; i < nets && i < 30; i++) {
    String s = WiFi.SSID(i);
    if (s == "") s = "(hidden)";
    netlist[0] += "<div class=d><span>" + s + "</span><span>CH" + WiFi.channel(i) + "</span></div>";
  }
...
void setup() {
  ...
  srv.on("/", root); srv.on("/scan", scan); srv.on("/atk", atk);
  srv.on("/bcn", bcn); srv.on("/ble", ble); srv.begin();
}
...
void loop() {
  ...
  if (attack && nets > 0) {
    ...
    digitalWrite(led, 0);
  } else if (beacon) { digitalWrite(led, 1); delay(30); digitalWrite(led, 0); delay(30); }
  else if (blespam) { digitalWrite(led, 1); delay(30); digitalWrite(led, 0); delay(30); }
